...
class GenerateVideoByWanVideoCfgpuInputSchema(BaseModel):
    prompt: str = Field(
...
@tool("generate_video_by_wan_video_cfgpu",
      description="Generate videos using Wan-Video model via CFGPU provider. Supports text-to-video generation.",
      args_schema=GenerateVideoByWanVideoCfgpuInputSchema)
async def generate_video_by_wan_video_cfgpu(
    prompt: str,
...
__all__ = ["generate_video_by_wan_video_cfgpu"]
